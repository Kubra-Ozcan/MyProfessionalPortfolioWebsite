...
import { ArrowLeft, ExternalLink, Github, Layers, Filter, Tag, BookOpen } from 'lucide-react';
...
const ProjectCard: React.FC<{ project: ProjectItem; category: ProjectCategory; labels: any }> = ({ project, category, labels }) => {
  ...
  return (
    <motion.div 
        ...
        className="group relative bg-white dark:bg-[#0f0f1a] border border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden transition-all duration-300 flex flex-col h-full hover:scale-[1.02] hover:border-purple-500/50 hover:shadow-[0_0_30px_rgba(168,85,247,0.15)] hover:z-10"
    >
        ...
        <div className="relative h-60 overflow-hidden bg-slate-200 dark:bg-gray-900/50">
            ...
            <img 
                ...
                className={`w-full h-full object-cover transform group-hover:scale-110 transition-all duration-700 ${isImageLoaded ? 'opacity-100 blur-0' : 'opacity-0 blur-sm'}`}
            />
        ...
        <div className="p-6 flex flex-col flex-grow relative z-20 -mt-12">
            ...
            <div className="flex flex-wrap gap-2 mb-8 mt-auto">
                {project.technologies.slice(0, 4).map((tech) => (
                    <span key={tech} className="px-2.5 py-1 text-[11px] font-semibold tracking-wide bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-gray-300 rounded-full border border-slate-200 dark:border-white/10 group-hover:border-purple-500/30 group-hover:text-purple-700 dark:group-hover:text-purple-200 transition-colors">
                        {tech}
                    </span>
                ))}
                {project.technologies.length > 4 && (
                    <span className="px-2.5 py-1 text-[11px] font-semibold bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-gray-500 rounded-full border border-slate-200 dark:border-white/10">
                    +{project.technologies.length - 4}
                    </span>
                )}
            </div>
            ...
            <div className="flex flex-col gap-3 mt-auto">
                ...
                <div className="grid grid-cols-2 gap-3">
                    {hasGithub ? (
                        ...
                        <button 
                            disabled
                            className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-gray-500 text-sm font-bold border border-slate-200 dark:border-white/5 cursor-not-allowed opacity-50"
                        >
                    ...
                    <button 
                        ...
                        className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-white text-sm font-bold border border-slate-200 dark:border-white/10 hover:bg-white dark:hover:bg-white/10 hover:border-purple-500/50 hover:text-purple-600 dark:hover:text-purple-400 hover:shadow-[0_0_15px_rgba(168,85,247,0.2)] transition-all duration-300 cursor-default active:scale-100"
                    >
...
const ProjectList: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-50 dark:bg-[#050511] overflow-y-auto custom-scrollbar transition-colors duration-300">
      ...
      {/* Sticky Header */}
      <div className="sticky top-0 z-50 bg-white/90 dark:bg-[#050511]/90 backdrop-blur-xl border-b border-slate-200 dark:border-white/10 px-6 py-4 flex items-center justify-between shadow-sm dark:shadow-2xl transition-all duration-300">
         <button 
            onClick={() => navigate('/')}
            className="flex items-center gap-2 text-slate-600 dark:text-gray-400 hover:text-black dark:hover:text-white px-4 py-2 rounded-full hover:bg-slate-100 dark:hover:bg-white/5 transition-all group border border-transparent hover:border-slate-200 dark:hover:border-white/10"
         >
      ...
      <div className="relative pt-6 pb-12 px-6 overflow-hidden">
          ...
          <div className="container mx-auto relative z-10 text-center max-w-4xl flex flex-col items-center">
              <motion.div 
                ...
                className="inline-flex p-5 rounded-2xl bg-white/60 dark:bg-white/5 backdrop-blur-md border border-slate-200 dark:border-white/10 shadow-lg mb-6 ring-1 ring-slate-100 dark:ring-white/10"
              >
            ...
            <button
              ...
              className={`
                ...
                ${activeFilter === tag 
                  ...
                  : 'bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-500 dark:text-gray-400 hover:bg-slate-50 dark:hover:bg-white/10 hover:border-slate-300 dark:hover:border-white/20 hover:text-slate-800 dark:hover:text-gray-200'
                }
              `}
            >
