          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {data.PROJECT_CATEGORIES.map((category, index) => (
              ...
              >
                ...
                <div className={`absolute inset-0 bg-gradient-to-br ${category.gradient} opacity-0 group-hover:opacity-20 dark:group-hover:opacity-40 rounded-3xl transition-opacity duration-500 blur-2xl`}></div>
                
                {/* Card Container */}
                <div className="relative h-full bg-white dark:bg-[#13132b]/80 backdrop-blur-xl border border-slate-200 dark:border-white/5 p-8 rounded-3xl transition-all duration-300 group-hover:-translate-y-2 group-hover:border-purple-500/30 dark:group-hover:border-white/20 group-hover:shadow-2xl flex flex-col overflow-hidden">
                  
                  {/* Decorative corner gradient blob */}
                  <div className={`absolute -top-10 -right-10 w-32 h-32 bg-gradient-to-br ${category.gradient} rounded-full blur-2xl opacity-10 dark:opacity-20 group-hover:opacity-30 dark:group-hover:opacity-40 transition-opacity duration-500`}></div>
                  ...
                  <div className="mt-auto pt-6 border-t border-slate-100 dark:border-white/5 flex justify-between items-center group-hover:border-slate-200 dark:group-hover:border-white/20 transition-colors relative z-10">
