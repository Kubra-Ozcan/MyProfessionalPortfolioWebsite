  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled 
          ? 'bg-white/80 dark:bg-dark/80 backdrop-blur-md py-4 shadow-lg border-b border-black/5 dark:border-white/5' 
          : 'bg-transparent py-6'
      }`}
    >
      <div className="container mx-auto px-6 flex justify-between items-center">
        {/* Logo */}
        <Link to="/" className="text-2xl font-bold tracking-tighter bg-gradient-to-r from-orange-400 to-purple-500 bg-clip-text text-transparent hover:scale-105 transition-transform font-mono">
          &lt; KUBRA OZCAN /&gt;
        </Link>
                ...
                <AnimatePresence>
                  {activeDropdown === 'projects' && (
                    <motion.div 
                      ...
                      className="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-slate-900/95 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-xl shadow-2xl overflow-hidden"
                    >
        ...
        <div className="hidden md:flex items-center gap-4 relative">
            ...
            <button
                onClick={toggleTheme}
                className="p-2 rounded-full bg-black/5 dark:bg-white/5 text-gray-600 dark:text-orange-400 hover:bg-black/10 dark:hover:bg-white/10 transition-colors border border-black/5 dark:border-white/10"
            ...
            >
                <button 
                  className="flex items-center gap-2 px-4 py-2 text-gray-700 dark:text-white bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors"
                >
                ...
                <AnimatePresence>
                  {isLangDropdownOpen && (
                    <motion.div 
                      ...
                      exit={{ opacity: 0, y: 10, scale: 0.95 }}
                      transition={{ duration: 0.2 }}
                      className="absolute top-full right-0 mt-2 w-32 bg-white dark:bg-slate-900/95 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-xl shadow-2xl overflow-hidden"
                    >
                       <div className="p-1">
                          <button 
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
            ...
            className="md:hidden bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t border-black/5 dark:border-white/10 overflow-hidden shadow-xl"
          >
